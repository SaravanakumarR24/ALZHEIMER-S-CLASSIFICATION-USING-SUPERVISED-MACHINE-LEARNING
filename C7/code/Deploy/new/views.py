...
def predict_view(request):
    if request.method == 'POST':
        print('IF')
        fieldss = ['Gender', 'Age', 'EDUC', 'SES', 'MMSE', 'CDR', 'eTIV', 'nWBV', 'ASF']
        ...
        if output == 0:
            ...
            ob.save()
            return render(request, 'new/index.html', {"prediction_text":"ALZHEIMER'S IS CONVERTED"})
        elif output == 1:
            ob = models.UserPredictDataModel.objects.latest('id')
            ob.output = "ALZHEIMER'S IS DEMENTED"
            ob.save()
            return render(request, 'new/index.html', {"prediction_text":"ALZHEIMER'S IS DEMENTED"})
        elif output == 2:
            ob = models.UserPredictDataModel.objects.latest('id')
            ob.output = "ALZHEIMER'S IS NON DEMENTED"
            ob.save()
            return render(request, 'new/index.html', {"prediction_text":"ALZHEIMER'S IS NON DEMENTED"})
